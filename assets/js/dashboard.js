...
// Class definition
var spDashboard = function() {
    
    var mediumCharts = function() {
        spLib.initMediumChart('sp_widget_mini_chart_1', [20, 45, 20, 10, 20, 35, 20, 25, 10, 10], 70, spApp.getStateColor('brand'));
        spLib.initMediumChart('sp_widget_mini_chart_2', [10, 15, 25, 45, 15, 30, 10, 40, 15, 25], 70, spApp.getStateColor('danger'));
        spLib.initMediumChart('sp_widget_mini_chart_3', [22, 15, 40, 10, 35, 20, 30, 50, 15, 10], 70, spApp.getBaseColor('shape', 4));
    }

    var latestProductsMiniCharts = function() {
        spLib.initMiniChart($('#sp_widget_latest_products_chart_1'), [6, 12, 9, 18, 15, 9, 11, 8], spApp.getStateColor('info'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_latest_products_chart_2'), [8, 6, 13, 16, 9, 6, 11, 14], spApp.getStateColor('warning'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_latest_products_chart_3'), [8, 6, 13, 16, 9, 6, 11, 14], spApp.getStateColor('warning'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_latest_products_chart_4'), [3, 9, 9, 18, 15, 9, 11, 8], spApp.getStateColor('success'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_latest_products_chart_5'), [5, 7, 9, 18, 15, 9, 11, 8], spApp.getStateColor('brand'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_latest_products_chart_6'), [3, 9, 5, 18, 15, 7, 11, 6], spApp.getStateColor('danger'), 2, false, false);
    }

    var generalStatistics = function() {
        // Mini charts
        spLib.initMiniChart($('#sp_widget_general_statistics_chart_1'), [6, 8, 3, 18, 15, 7, 11, 7], spApp.getStateColor('warning'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_general_statistics_chart_2'), [8, 6, 9, 18, 15, 7, 11, 16], spApp.getStateColor('brand'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_general_statistics_chart_3'), [4, 12, 9, 18, 15, 7, 11, 12], spApp.getStateColor('danger'), 2, false, false);
        spLib.initMiniChart($('#sp_widget_general_statistics_chart_4'), [3, 14, 5, 12, 15, 8, 14, 16], spApp.getStateColor('success'), 2, false, false);

        // Main chart
        if (!document.getElementById("sp_widget_general_statistics_chart_main")) {
            return;
        }

        var ctx = document.getElementById("sp_widget_general_statistics_chart_main").getContext("2d");

        var gradient1 = ctx.createLinearGradient(0, 0, 0, 350);
        gradient1.addColorStop(0, Chart.helpers.color(spApp.getStateColor('brand')).alpha(0.3).rgbString());
        gradient1.addColorStop(1, Chart.helpers.color(spApp.getStateColor('brand')).alpha(0).rgbString());

        var gradient2 = ctx.createLinearGradient(0, 0, 0, 350);
        gradient2.addColorStop(0, Chart.helpers.color(spApp.getStateColor('danger')).alpha(0.3).rgbString());
        gradient2.addColorStop(1, Chart.helpers.color(spApp.getStateColor('danger')).alpha(0).rgbString());
        ...
        var mainConfig = {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Sales',
                    borderColor: spApp.getStateColor('brand'),
                    borderWidth: 2,
                    ...
                    label: 'Orders',
                    borderWidth: 1,
                    borderColor: spApp.getStateColor('danger'),
                    backgroundColor: gradient2,
                    pointBackgroundColor: spApp.getStateColor('danger'),
    ...
    var widgetTechnologiesChart = function() {
        if ($('#sp_widget_technologies_chart').length == 0) {
            return;
        }
        ...
        var config = {
            ...
            data: {
                datasets: [{
                    ...
                    backgroundColor: [
                        spApp.getBaseColor('shape', 3),
                        spApp.getBaseColor('shape', 4),
                        spApp.getStateColor('brand')
                    ]
                }],
                labels: [
                    'Angular',
        ...
        var ctx = document.getElementById('sp_widget_technologies_chart').getContext('2d');
        var myDoughnut = new Chart(ctx, config);
    }

    var widgetTechnologiesChart2 = function() {
        if ($('#sp_widget_technologies_chart_2').length == 0) {
            return;
        }
        ...
        var config = {
            ...
            data: {
                datasets: [{
                    ...
                    backgroundColor: [                                                                 
                        spApp.getStateColor('warning'),
                        spApp.getStateColor('brand'),
                        spApp.getStateColor('success')
                    ]
                }],
                labels: [       
                    'CSS',     
        ...
        var ctx = document.getElementById('sp_widget_technologies_chart_2').getContext('2d');
        var myDoughnut = new Chart(ctx, config);
    }

    var widgetTotalOrdersChart = function() {
        if (!document.getElementById('sp_widget_total_orders_chart')) {
            return;
        }

        // Main chart
        var max = 80;
        var color = spApp.getStateColor('brand');
        var ctx = document.getElementById('sp_widget_total_orders_chart').getContext("2d");
    ...
    var widgetTotalOrdersChart2 = function() {
        if (!document.getElementById('sp_widget_total_orders_chart_2')) {
            return;
        }

        // Main chart
        var max = 80;
        var color = spApp.getStateColor('danger');
        var ctx = document.getElementById('sp_widget_total_orders_chart_2').getContext("2d");
        var gradient = ctx.createLinearGradient(0, 0, 0, 120);
        gradient.addColorStop(0, Chart.helpers.color(color).alpha(0.3).rgbString());
        gradient.addColorStop(1, Chart.helpers.color(color).alpha(0).rgbString());
    ...
    var widgetSalesStatisticsChart = function() {
        if (!document.getElementById('sp_chart_sales_statistics')) {
            return;
        }
        ...
        var barChartData = {
            ...
            datasets: [{
                label: 'Sales',
                backgroundColor: color(spApp.getStateColor('brand')).alpha(1).rgbString(),
                borderWidth: 0,
                data: [20, 30, 40, 35, 45, 55, 45]
            }, {
                label: 'Orders',
                backgroundColor: color(spApp.getBaseColor('shape', 1)).alpha(1).rgbString(),
                borderWidth: 0,
                data: [25, 35, 45, 40, 50, 60, 50]
            }]
        };

        var ctx = document.getElementById('sp_chart_sales_statistics').getContext('2d');
                ...
                scales: {
                    xAxes: [{
                        ...
                        ticks: {
                            display: true,
                            beginAtZero: true,
                            fontColor: spApp.getBaseColor('shape', 3),
                            fontSize: 13,
                            padding: 10
                        }
                    }],
                    yAxes: [{
                        ...
                        gridLines: {
                            color: spApp.getBaseColor('shape', 2),
                            drawBorder: false,
                            offsetGridLines: false,
                            drawTicks: false,
                            borderDash: [3, 4],
                            zeroLineWidth: 1,
                            zeroLineColor: spApp.getBaseColor('shape', 2),
    ...
    var widgetRevenueGrowthChart = function() {
        if (!document.getElementById('sp_chart_revenue_growth')) {
            return;
        }
        ...
        var barChartData = {
            ...
            datasets: [{
                ...
                borderColor: spApp.getStateColor('brand'),
                borderWidth: 3,
                backgroundColor: color(spApp.getStateColor('brand')).alpha(0.07).rgbString(),
                //pointBackgroundColor: spApp.getStateColor('brand'),
                fill: true
            }]
        };

        var ctx = document.getElementById('sp_chart_revenue_growth').getContext('2d');
        var myBar = new Chart(ctx, {
            type: 'line',
            data: barChartData,
            options: {
    ...
    var daterangepickerInit = function() {
        if ($('#sp_dashboard_daterangepicker').length == 0) {
            return;
        }

        var picker = $('#sp_dashboard_daterangepicker');
        var start = moment();
        var end = moment();
        ...
        function cb(start, end, label) {
            ...
            picker.find('#sp_dashboard_daterangepicker_date').html(range);
            picker.find('#sp_dashboard_daterangepicker_title').html(title);
        }

        picker.daterangepicker({
            direction: spUtil.isRTL(),
            startDate: start,
            endDate: end,
            opens: 'left',
    ...
    var recentOrdersInit = function() {
        ...
        var dataJSONArray = [{"RecordID":1,"OrderID":"61715-075","Country":"China","ShipCountry":"CN","ShipCity":"Tieba","ShipName":"Collins, Dibbert and Hoeger","ShipAddress":"746 Pine View Junction","CompanyEmail":"nsailor0@livejournal.com","CompanyAgent":"Nixie Sailor","CompanyName":"Gleichner, Ziemann and Gutkowski","Currency":"CNY","Notes":"imperdiet nullam orci pede venenatis non sodales sed tincidunt eu felis fusce posuere felis sed lacus morbi","Department":"Outdoors","Website":"irs.gov","Latitude":35.0032213,"Longitude":102.913526,"ShipDate":"2/12/2018","PaymentDate":"2016-04-27 23:53:15","TimeZone":"Asia/Chongqing","TotalPayment":"$246154.65","Status":3,"Type":2,"Actions":null},
            ...
            {"RecordID":5,"OrderID":"31722-529","Country":"Austria","ShipCountry":"AT","ShipCity":"Sansp Andrä-Höch","ShipName":"Stehr-Kunde","ShipAddress":"3038 Trailsway Junction","CompanyEmail":"tst4@msn.com","CompanyAgent":"Tierney St. Louis","CompanyName":"Dicki-Kling","Currency":"EUR","Notes":"felis fusce posuere felis sed lacus morbi sem mauris laoreet ut rhoncus aliquet pulvinar sed nisl nunc rhoncus dui","Department":"Health","Website":"jimdo.com","Latitude":46.791555,"Longitude":15.379192,"ShipDate":"5/20/2017","PaymentDate":"2016-02-17 10:53:48","TimeZone":"Europe/Vienna","TotalPayment":"$516467.41","Status":2,"Type":3,"Actions":null},
        ...
        var datatable = $('#sp_recent_orders').spDatatable({
            data: {
                type: 'local',
            ...
            columns: [{
                ...
                template: function(data, i) {
                    ...
                    var output = '\
                        <div class="sp-user-card-v2">\
                            <div class="sp-user-card-v2__pic">\
                                <img src=" acá direccion url ' + img + '" alt="photo">\
                            </div>\
                            <div class="sp-user-card-v2__details">\
                                <a href="#" class="sp-user-card-v2__name">' + data.CompanyName + '</a>\
                                <span class="sp-user-card-v2__email">' +
                ...
                template: function(data) {
                    return '<span class="sp-font-bold">' + data.ShipDate + '</span>';
                }
            }, {
                field: "Status",
                title: "Status",
                ...
                template: function(data, i) {
                    ...
                    var pos = spUtil.getRandomInt(0, 5);
                    var position = [
                        'Developer',
                        'Designer',
                        'CEO',
                    ...
                    var output = '';
                    if (number > 5) {
                        output = '<div class="sp-user-card-v2">\
							<div class="sp-user-card-v2__pic">\
								<img src=" url de la img' + user_img + '" alt="photo">\
							</div>\
							<div class="sp-user-card-v2__details">\
								<a href="#" class="sp-user-card-v2__name">' + data.CompanyAgent + '</a>\
								<span class="sp-user-card-v2__desc">' + position[pos] + '</span>\
							</div>\
						</div>';
                    }
                    else {
                        var stateNo = spUtil.getRandomInt(0, 6);
                        var states = [
                            'success',
                            'brand',
                            'danger',
                        ...
                        output = '<div class="sp-user-card-v2">\
							<div class="sp-user-card-v2__pic">\
								<div class="sp-badge sp-badge--xl sp-badge--' + state + '">' + data.CompanyAgent.substring(0, 1) + '</div>\
							</div>\
							<div class="sp-user-card-v2__details">\
								<a href="#" class="sp-user-card-v2__name">' + data.CompanyAgent + '</a>\
								<span class="sp-user-card-v2__desc">' + position[pos] + '</span>\
                            ...
                            <div class="dropdown-menu dropdown-menu-right">\
                                <ul class="sp-nav">\
                                    <li class="sp-nav__item">\
                                        <a href="#" class="sp-nav__link">\
                                            <i class="sp-nav__link-icon flaticon2-expand"></i>\
                                            <span class="sp-nav__link-text">View</span>\
                                        </a>\
                                    </li>\
                                    <li class="sp-nav__item">\
                                        <a href="#" class="sp-nav__link">\
                                            <i class="sp-nav__link-icon flaticon2-contract"></i>\
                                            <span class="sp-nav__link-text">Edit</span>\
                                        </a>\
                                    </li>\
                                    <li class="sp-nav__item">\
                                        <a href="#" class="sp-nav__link">\
                                            <i class="sp-nav__link-icon flaticon2-trash"></i>\
                                            <span class="sp-nav__link-text">Delete</span>\
                                        </a>\
                                    </li>\
                                    <li class="sp-nav__item">\
                                        <a href="#" class="sp-nav__link">\
                                            <i class="sp-nav__link-icon flaticon2-mail-1"></i>\
                                            <span class="sp-nav__link-text">Export</span>\
                                        </a>\
                                    </li>\
                                </ul>\
                            </div>\
        ...
        $('#sp_form_status').on('change', function() {
            datatable.search($(this).val().toLowerCase(), 'status');
        });

        $('#sp_form_type').on('change', function() {
            datatable.search($(this).val().toLowerCase(), 'type');
        });

        $('#sp_form_status,#sp_form_type').selectpicker();

        // Reload datatable layout on aside menu toggle
        if (spLayout.getAsideSecondaryToggler && spLayout.getAsideSecondaryToggler()) {
            spLayout.getAsideSecondaryToggler().on('toggle', function() {
                datatable.redraw();
            });
        }
    }
